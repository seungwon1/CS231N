...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores1 = np.exp(score) / np.reshape(np.sum(np.exp(score), axis = 1), (score.shape[0], 1))
    scores1[np.arange(scores1.shape[0]), y] -= 1
    
    dW2 = np.dot(first_hidden_units.T, scores1) / X.shape[0]
    dW2 += reg*(W2)
    db2 = np.sum(scores1, axis = 0) / X.shape[0]
    grads['W2'] = dW2
    grads['b2'] = db2
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      X_batch_indicies = np.random.choice(num_train, batch_size)
      X_batch = X[X_batch_indicies, :]
      y_batch = y[X_batch_indicies]
      #########################################################################
      #                             END OF YOUR CODE                          #
      #########################################################################
